...
class Window: #create a tkinter window
    def __init__(self,width,height):
        self.root = Tk()
        self.root.title("Maze")
        self.root.protocol("WM_DELETE_WINDOW", self.close) #handle close event
        self.canvas = Canvas(height=height,width=width) #make it a canvas to draw the maze
        self.canvas.pack()
        self.running = False

    def redraw(self): # updates the ui
        self.root.update_idletasks()
        self.root.update()


    def wait_for_close(self): # continues update
        self.running = True
        while self.running:
            self.redraw()
...
class cell: # a cell in a maze
    ...
    def break_entrance(self): #for first cell 
        self.has_top_wall = False
    
    def break_exit(self): #for last cell
        self.has_bottom_wall = False

    def draw(self, x1, y1, x2, y2): #draw the cell
    ...
    def draw_move(self, to_cell, undo = False): #draw the moves in red
        color = "red"
        if undo:
            color = "grey"
...
class Maze:
    ...
    def _create_cells(self):
        for i in range(self.num_cols):  # Loop through each column
            col_cells = []
            for j in range(self.num_rows):  # Loop through each row
                col_cells.append(cell(self.win))  # Create a cell
            self._cells.append(col_cells)  # Add the column to the maze

        for i in range(self.num_cols):
            for j in range(self.num_rows):
                self._draw_cells(i, j)  # Draw each cell

    def _draw_cells(self,i,j):
        if self.win is None:
            return
        x1 = self.x1 + i * self.cell_size_x  #gets the cell cordinates
        y1 = self.y1 + j * self.cell_size_y
        x2 = x1 + self.cell_size_x
        y2 = y1 + self.cell_size_y
        self._cells[i][j].draw(x1,y1,x2,y2) #draw a cell
        self._animate() #refresh window
    ...
    def _Break_walls_R(self, i, j):
        ...
        while True:  #checks all four direction for unchecked cells
            next_index_list = []
            ...
            if len(next_index_list) == 0: # if no more cells to visit, backtrack
            ...
            if next_index[0] == i+1: #break walls between neignbors
